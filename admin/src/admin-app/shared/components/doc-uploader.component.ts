...
template: ` <a class="custom-file-btn delete-file" *ngIf="fileName1 && isValidFile" href="javascript:void(0)" (click)="onDeleteFile()">
            <i class="fa fa-trash" aria-hidden="true"></i> Remove</a>
            <div class="custom-file-wrap">
              <input type="file" class="custom-file-input" [id]="'file'+id" #inputFile (change)="onFileSelect($event)">
              <label class="custom-file-btn" for="file"><i class="fa fa-upload" aria-hidden="true"> {{fileName1?fileName1:"Choose a File..."}}</i></label>
            </div>
