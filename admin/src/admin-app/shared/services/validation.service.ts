...
export class ValidationService {
  ...
  static getValidatorErrorMessage(code: string, customValue?: string) {
    let config = {
      ...
      'age':"Age should be greater then 13"
  ...
  static ageValidation(control: FormControl) {
    ...
    var dob=new Date(num);
    var dobyear=dob.getFullYear();
    var age=curyear-dobyear
    var regex = /^(\d|\.)*$/;
    console.log('numnumnumnumnum',num,age)
    if (num && age<13) {
      return {
        'age': {valid: false}
      };
  ...
  static ageValidations(control: FormControl) {
    var num = control.value;
    var currdate=new Date();
    var curyear=currdate.getFullYear();
    var dob=new Date(num.formatted);
    var dobyear=dob.getFullYear();
    var age=curyear-dobyear
    var regex = /^(\d|\.)*$/;
    console.log('numnumnumnumnum',num,age)
 ...
 static numberValidator(control: FormControl) {
  ...
  static  documentValidation(file: File, allowedExt: string[], allowedSizeInMB: number) {
  ...
  static  imageValidation(file: File, allowedExt: string[], allowedSize: number) {
    let fileName = file.name;
    let fileSize = file.size;
    let fileExt = fileName.substr(fileName.lastIndexOf('.') + 1).toLowerCase();
    let validExt: string[] = allowedExt.map(function (row) {
