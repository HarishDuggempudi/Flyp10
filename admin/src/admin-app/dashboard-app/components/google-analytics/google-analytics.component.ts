...
export class GoogleAnalyticsComponent implements OnInit {
  ...
  saveAnalytics() {
    ...
    if (this.analyticsForm.valid) {
      console.log('inside valid analytics form')
      let objAnalyticsSave = new GoogleAnalyticsModel();
      console.log('obj analytics ', this.objAnalytics);
      console.log('obj AnalyticsSave ', objAnalyticsSave);
      ...
      if (!this.objAnalytics._id) {
        ...
        this._objService.saveGoogleAnalytics(objAnalyticsSave, this.file)
          .subscribe(res => {
            console.log('response from saving ganal');
            return this.resStatusMessage(res)
          },
            error => this.errorMessage(error));
      }
      else {
        console.log('inside false ', this.isSubmitted);
        this.isPost = false;
  ...
  onFileSelect(args: any) {
    console.log('on file select clicked ', args);
    this.file = args;
    if (this.file)
