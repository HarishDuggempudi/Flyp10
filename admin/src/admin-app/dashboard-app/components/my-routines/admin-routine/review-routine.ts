...
@Component({
  selector: "review-routine-list",
  templateUrl: "./review-routine.html",
  styleUrls: ['../routine-list.scss']
})
export class ReviewRoutineListComponent implements OnInit {
    displayedColumns = ['SN', 'Title','RoutineId','EventMeet','Athlete','Judge','Status','UploadedOn','Actions'];    
    dataSource: any;    
    userId: string;
    ...
    constructor(private _objService: EventMeetService,private eventService:EventMeetService,private route:Router,private _Service:AdminRoutineService,private walletservice: WalletService,private sportService:SportService){
    ...
    ngOnInit() {
      ...
      applyFilter(filterValue: string) {
        ...
        reviewlist:any=[]
        getReviewRoutine(){
            this.eventService.getReviewRoutine().subscribe(res=>{
                //console.log("getReviewRoutine",res)
                if(res.success){
                    this.reviewlist=res.result
                    this.dataSource = new MatTableDataSource(res.result);
                }else{
                    Swal("Alert","Unable to load review  routine details.","info")
                }
            },err=>{
                Swal("Alert","Unable to load review  routine details.","info")
            })
        ...
        delete(id: string,routine) {
          ...
          reset(id: string,routine) {
            Swal({
              title: "Are you sure?",
              text: "You want to move this routine to queue again.",
              type: "warning",
              showCancelButton: true,
              confirmButtonColor: "#DD6B55",
              ...
              if (result.value) {
                ...
                objTemp.jid=routine.judgeInfo._id
                objTemp.deleted = true;
                this.eventService.updateMeetdeventRoutineStatus(objTemp).subscribe(
                  res => {
                    this.getReviewRoutine();
