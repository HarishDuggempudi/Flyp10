...
export class USAGYMMemberComponent implements OnInit {
  ...
  isUSAGMember = false;
    ...
    constructor(private _objUserService:UserService, private route: ActivatedRoute,private sportService:SportService, private router: Router,private _formBuilder:FormBuilder) {
    ...
    updateUSAGMember(){
        ...
        this._objUserService.updateUSAGMember(this.userId,this.memberObj).subscribe((res)=>{
            ...
            if(res.success) {
                ...
                Swal("Alert !", "MemberID not verified.  Must enter MemberID associated with adult email on your USAG membership’", "info");
                this.router.navigate(['/profile']);
            } 
        })
    }
