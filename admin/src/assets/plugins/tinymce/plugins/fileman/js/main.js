...
function fileUpload(f, i) {
    ...
    // fData.append("documentName", $('#hdDir').attr('value'));
    ...
    http.setRequestHeader("Authorization", window.localStorage.getItem("NodeBeatAuthToken"));
    http.send(fData);
}
...
function tooltipContent() {
    ...
    if ($('#hdViewType').val() == 'thumb' && f.IsImage()) {
        ...
        if (RoxyFilemanConf.GENERATETHUMB) {
            // imgUrl =  RoxyFilemanConf.GENERATETHUMB; // RoxyUtils.AddParam(RoxyFilemanConf.GENERATETHUMB, 'f', f.fullPath);
            imgUrl = f.fullPath.substring(0, f.fullPath.lastIndexOf('/') + 1) + RoxyFilemanConf.GENERATETHUMB + f.fullPath.substring(f.fullPath.lastIndexOf('/') + 1); //RoxyUtils.AddParam(RoxyFilemanConf.GENERATETHUMB, 'f', imgUrl);
    ...
    if (t == 'thumb') {
        ...
        $('#pnlFileList li').each(function () {
            ...
            if (RoxyFilemanConf.GENERATETHUMB && RoxyUtils.IsImage($(this).attr('data-path'))) {
                ...
                imgUrl = imgUrl.substring(0, imgUrl.lastIndexOf('/') + 1) + RoxyFilemanConf.GENERATETHUMB + imgUrl.substring(imgUrl.lastIndexOf('/') + 1); //RoxyUtils.AddParam(RoxyFilemanConf.GENERATETHUMB, 'f', imgUrl);
