        ...
        if (errors) {
            ...
            return dataProviderHelper.checkForDuplicateEntry(Routine, query).then(function(count) {
                    if (count > 0) {
                        ...
                        var newRoutine = RoutineModule.CreateRoutine(modelInfo, req.body.eventMeetId._id, req.decoded.user.username, routineInfo, convertedfileName);
                        //console.log('newroutine',newRoutine)
                        return [newRoutine, dataProviderHelper.save(newRoutine).then(routineResponse => {

                            if (req.body.uploadingType == '2') {
                                for (let i = 0; i < req.body.judges.length; i++) {
                                    let body = {
                                        eventId: req.body.eventMeetId._id,
                                        judgeId: req.body.judges[i],
