...
//comment = comment.replace(/&#039;/g, "'");
    ...
    _p.eventsScoreById = function (req, res, next) {
            ...
            query = [
                {
                    $match: {
                        $and: [{
                            "uid": { $in: uidArr },
                            "deleted": false, "routinestatus": "1", "judgedOn": { "$gte": forDays }
                        }]
                    }
                },
        ...
        Routine.aggregate(query, function (err, result) {
            if (err) console.log('err running query --> ', err)
            console.log('result ', result);
            res.send(result)
        })
    }
...
console.log(eventMeetId.EventLevel,"eventlevel")
   ...
   //console.log(id,"judgeId")
        ...
        dataProviderHelper.aggregate(NotificationTokenObj, query).then(function (notification) {
            ...
            if (notification.length > 0) {
                            console.log(notification[0].token);
                            console.log(notification[0].UserInformation.username )
                            let body = {

                                "notification": {
                                    "title": "Flyp10 routines available",
                                    "body": notification[0].UserInformation.username + ',  A new routine that meet your profile and are waiting for a score on Flyp10.',
                                    "sound": "default",
                                    "click_action": "FCM_PLUGIN_ACTIVITY",
                                    "icon": "fcm_push_icon"
                                },
