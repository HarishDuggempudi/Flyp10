var passwordChangeVerifyTokenController = (function() {
        ...
        return _p.checkTokenExpiryStatus(req, next)
            ...
            .spread(function(returnVal, dataObj) {
                if (returnVal.expired) {
                    _p.sendEmailToConfirmPasswordChangeAction(req, dataObj.email, dataObj._id, dataObj, next);
    ...
    _p.sendEmailToConfirmPasswordChangeAction = function(req, userEmail, userID, userObj, next) {
        return new Promise(function(resolve, reject) {
            hasher.generateRandomBytes(36)
                ...
                .spread(function(tokenBytes, emailTemplate) {
                    ...
                    var url = 'https://flyp10.com/admin/change-password/' + tokenBytes;
                    var messageBody = '';
                    if (emailTemplate && emailTemplate.templateBody) {
                        ...
                        //console.log("usserobj",userObj);
                        if (userObj.username) {
                            messageBody = messageBody.replace("{{username}}", userObj.username);
                            messageBody = messageBody.replace("{{name}}", userObj.firstName + " " + userObj.lastName);
