var teamMemberRoutes = (function() {
    ...
    var HTTPStatus = require('http-status'),
        ...
        imageFilePath = '/mnt/volume_sfo2_01/public/uploads/images/teammember/',
    ...
    teamMemberRouter.route('/teammate')
        .get(getAllTeammates)
        .post(tokenAuthMiddleware.authenticate, roleAuthMiddleware.authorize, teamMemberController.addTeamMate)

    teamMemberRouter.route('/acceptedByUID/:UID')
        .get(teamMemberController.getAcceptedByUID)

    teamMemberRouter.use('/teammates/:UID', function(req, res, next) {
        teamMemberController.getTeammatesById(req)
            .then(function(teamMateInfo) {
                if (teamMateInfo) {
                    req.teamMateInfo = teamMateInfo;
                    //console.log('teamMateInfo', teamMateInfo[0]);
                    res.json({
                        reqData: teamMateInfo
                    });
                    // next();
                    // return null;
                } else {
    ...
    teamMemberRouter.use('/teammatedetails/:username', function(req, res, next) {
        teamMemberController.getTeamMemberInfoByUsername(req)
            .then(function(teamMateInfo) {
                if (teamMateInfo) {
                    req.teamMateInfo = teamMateInfo;
                    //console.log('teamMateInfo', teamMateInfo[0]);
    ...
    teamMemberRouter.use('/teammatereq/:FID', function(req, res, next) {
        teamMemberController.getRequestsByFID(req)
            .then(function(teamMateInfo) {
                if (teamMateInfo) {
                    req.teamMateInfo = teamMateInfo;
                    //console.log('teamMateInfo', teamMateInfo[0]);
    ...
    teamMemberRouter.use('/teammate/:reqid', function(req, res, next) {
        teamMemberController.getTeammateById(req)
            .then(function(teamMateInfo) {
                if (teamMateInfo) {
                    req.teamMateInfo = teamMateInfo;
                    //console.log('teamMateInfo', teamMateInfo);
                    next();
                    return null;
                } else {
                    res.status(HTTPStatus.NOT_FOUND);
    ...
    teamMemberRouter.route('/teammate/:UID')
        .get(function(req, res) {
            res.status(HTTPStatus.OK);
            res.json(req.teamMateInfo);
        })
        .put(tokenAuthMiddleware.authenticate, roleAuthMiddleware.authorize, teamMemberController.updateTeamMateInfo);
    ...
    function getAllTeammates(req, res, next) {
        //console.log('passphrase is ',passphrase);
        teamMemberController.getAllTeammates(req, next)
            .then(function(teamMemberList) {
                //if exists, return data in json format
                if (teamMemberList) {
