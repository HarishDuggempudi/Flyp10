var blogRoutes = (function() {
    ...
    var HTTPStatus = require('http-status'),
        ...
        imageFilePath = '/mnt/volume_sfo2_01/public/uploads/images/blogs/',
        documentFilePath = '/mnt/volume_sfo2_01/public/uploads/documents/blogs/',
        uploadPrefix = 'blog',
    ...
    blogRouter.route('/blogs/rss')
        .get(getBlogRssfeed);



    function getBlogRssfeed(req, res, next) {
        blogController.getBlogRssfeed(req, next)
            .then(function(rssfeed) {
                //if exists, return data in json format
                if (rssfeed) {
                    var feed = new rss({
                        title: 'Flyp10 Blog',
                        description: 'Get a judge’s score, using only your skills and a mobile device.' +
                            'Upload a recording of a performance or routine, we’ll get you a score and its supporting details from a certified judge in your sport. Build a library of achievements and highlights. Share all of your new scores with your coaches, friends,' +
                            'family, and teammates.' +
                            'Explore our analytics. Track your development and compare against nationwide trends.',
                        feed_url: 'http://' + req.headers.host + req.url,
                        site_url: 'http://' + req.headers.host,
                        image_url: 'http://' + req.headers.host + '/images/icon.png',
                        author: 'John Doe'
                    });
                    rssfeed.forEach(function(item, post) {
                        //console.log(item,post)
                        feed.item({
                            title: item.blogTitle,
                            description: item.blogDescription,
                            custom_elements: [
                                { summery: item.blogSummery }
                            ],

                            url: 'http://' + req.headers.host + '/blog/' + item._id + '/' + item.urlSlog,
                            author: item.author,
                            date: item.addedOn,
                            enclosure: {
                                url: 'http://' + req.headers.host + "/public/uploads/images/blogs/" + item.bannerImage,
                                type: item.imageProperties.imageExtension
                            }
                        });
                    });
                    res.status(HTTPStatus.OK);
